...
use windows101db;
 ...
 select department,sum(salary) from employee_performance;
...
select* from employee_performance order by sales_amount;-- oreder by 
select*, sum(sales_amount) over(order by sales_amount) from employee_performance ;-- cumulative sum
...
,sum(sales_amount) over(partition by location order by sales_amount)
 from employee_performance ;-- first it will give sum salary according to the set of row,then salary according to location(sum),adding the salary according to the sales_amount
 select*, sum(sales_amount) over(),sum(sales_amount) over(partition by location) as akash
,sum(sales_amount) over(partition by location order by sales_amount desc ) as ayush
 from employee_performance;-- desc
 create table employ_6 as select employee_id,employee_name,location ,round(sales_amount) as ayush from 
 employee_performance limit 5 ;
 select * from employ_6;
 select*,sum(ayush) over(partition by employee_name) from employ_6; 
  select*,sum(ayush) over(order  by employee_name) from employ_6;
   select*,sum(ayush) over(order by employee_id) from employ_6; 
   select*,sum(ayush) over(order by location) from employ_6;-- partition by set off rows mein hoga operation 
    select*,sum(ayush) over(partition by location order by ayush) from employ_6;
    select*,sum(ayush) over(partition by location order by employee_name ) from employ_6;-- phele loaction pe hoga partition by then uske ander oreder by hoga name pe 
    select employee_id 
